...
사용자는 최종 네 개 화면을 순서대로 이동합니다.

1. **거래 분석**: PDF·PNG·JPG/JPEG 거래 문서를 올리거나 합성 샘플을 선택하고,
   회사 역할·거래 방향·통화·금액·결제일을 원문과 대조해 확정합니다.
2. **환율 전망·위험**: 환율 방향·중심값·범위와 적용 horizon을 확인한 뒤, 검증된
   고정 스트레스가 원화 수취·지급과 현금 버퍼에 미치는 영향을 봅니다.
3. **금융지원 추천**: 선적 상태, 채권 존재, 자금 목적과 정책금융 의사를 필요한
   범위에서 확인하고 한 번의 저장 동작으로 공식 상담 후보를 갱신합니다.
4. **상담 준비·보고서**: 최신 후보와 상담 Top 3, 준비자료, 기관 질문을 확인하고
   동일 `ConsultationPacket`에서 만든 상담 준비서를 내려받습니다.

상품 추가 확인값은 거래에 결속된 `OfficialCandidateInputProfile` 한 곳에만 저장됩니다.
명시적으로 확인된 조건이 기존의 넓은 상담 신호보다 우선하며, 상충 입력은 추천 갱신
전에 차단합니다. 공식 금융후보 Top 3, Top 3 밖의 적격 후보, 정보가 부족한 후보와
신용조사 같은 보조 서비스는 서로 다른 영역으로 표시합니다.

내부 provider·실행 trace·JSON·fingerprint·critic 결과는 검증 코드에는 보존하되
기본 사용자 화면에는 노출하지 않습니다. 필요한 개발 정보는 개발 환경에서만 별도로
확인합니다.

확정 거래의 한국–상대국 월별 수출·수입·무역수지는 별도의 **거래국 무역 통계**
카드에서 확인합니다. Golden 한국–브라질 데모는 관세청 공식 공개 원자료를 고정한
API-free snapshot을 사용하고, 실제 업로드는 관세청 OpenAPI key가 있을 때만 live로
조회합니다. HS Code는 사용자가 원자료에서 확인한 2·4·6·10자리 값만 사용하며
상품명에서 추측하지 않습니다. 통계는 시장 맥락일 뿐 환율·현금흐름·헤지·상담
순위를 변경하지 않습니다. 상세 계약은 [docs/TRADE_STATISTICS.md](docs/TRADE_STATISTICS.md)에
있습니다.
...
화면에 연결된 뉴스는 환율 전망을 이해하기 위한 저장된 시장 배경입니다. 가격 예측
입력이나 수치 계산 근거가 아니며 금융 숫자를 변경하지 않습니다. 연결 자료가 없거나
upstream이 unavailable이어도 환율·현금흐름 분석은 정상적으로 계속됩니다.
