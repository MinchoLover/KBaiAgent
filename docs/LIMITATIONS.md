...
- 합성 데이터 중심이며 실제 OCR 품질·언어·레이아웃 분포의 live baseline은 API 키가
  있어야 측정할 수 있습니다.
- 이미지형 PDF는 로컬 `pypdf` 텍스트 레이어가 비어 있고 독립 OCR 엔진은 아직
  없습니다. Responses vision이 반환한 인용은 값 일치와 field 규칙을 검증하지만
  별도 OCR의 독립 확인은 아니므로 사용자가 원문을 최종 대조해야 합니다. 인용이
  누락되면 `MISSING_CORE_EVIDENCE`와 `OCR_REQUIRED`로 계산 전달을 차단합니다.
