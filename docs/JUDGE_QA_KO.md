...
## 23. amount_due는 실제 현재 미수·미지급 잔액인가요?

아닙니다. `grand_total`은 문서에 명시된 계약 또는 청구 총액이고,
`amount_due`는 Stage 2 분석에 투입되는 계약상 미결제 예정 노출액입니다. 화면에서는
수출이면 `분석 대상 예정 수취액`, 수입이면 `분석 대상 예정 지급액`으로 표시합니다.
Golden 계약서는 선지급 USD 20,000과 잔금 USD 80,000의 이행 여부를 알려주지 않아
두 예정 installment 합계 USD 100,000을 유지합니다.
