...
| 3. 핵심 거래값 사용자 확인 | `KR/BR`, `EXPORT`, `USD`, `100000.00`, `2026-08-20` 확인 후 확인 버튼 | `분석 대상 예정 수취액 USD 100,000`; 실제 미수잔액 `UNKNOWN` | “계약서의 예정 결제액을 분석하며, 실제 입금 이력이 확인되면 이행액을 제외해야 합니다.” | expected data를 읽어 설명하고 앱의 `수출기업 대표 데모`로 계산 화면 전환 |
...
| 9. 상담 Top 3 확인 | `2 위험 진단` 또는 `4 상담자료` 상단의 카드 확인 | 1순위 회수 보호 → 2순위 환율 → 3순위 운영자금 버퍼; 각 카드의 숫자·부족정보·기대 결정·다음 행동 | “이 순서는 AI 추천이나 승인등급이 아니라 기존 위험 finding의 결정론적 검토 순서입니다.” | `tests.test_consultation_priority`와 JSON packet 출력 제시 |
...
| 12. 한 페이지 handoff 확인 | `4 상담자료` → Top 3와 `상담 패킷 다운로드` | 거래·회차·보호수단·Top 3·준비자료·질문·공식후보·trace가 같은 JSON에서 파생 | “다운로드는 상담 준비자료이며 예약·RM 전송·신청 완료가 아닙니다.” | 결정론 Markdown handoff와 Stage 5 fallback 표시 |
