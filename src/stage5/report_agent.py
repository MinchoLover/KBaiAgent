...
consultation.trade_statistics가 있으면 국가 전체 또는 확인된 HS 품목 범위,
관측기간, 최근 12개월 수출·수입·무역수지, 직전 기간 대비 값, 공식 출처와
한계를 별도 "거래국 무역 통계" 섹션에 표시하라. 통계가 없으면 status와
이용 불가 사유를 표시하되 보고서 생성을 실패시키지 마라.
HS Code가 null이면 상품명에서 추측하지 말고 국가 전체 통계만 제공한다고 써라.
무역통계로 거래처 신용위험·환율 방향·헤지 비율·상담 순위·금융상품 승인
가능성을 만들거나 변경하지 마라.
